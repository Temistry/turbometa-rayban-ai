...
- DAT SDK 0.5.0이 지원하는 최대 스트림인 세로 720×1280·30fps를 고정 사용합니다. 높은 전송량 때문에 안경 배터리·발열과 iPhone 인코딩 부하는 증가할 수 있습니다.
- 사진은 DAT SDK가 제공한 약 1080×1440 JPEG 원본을 byte-for-byte로 보호 저장합니다. SDK에는 12MP 사진 해상도 선택 API가 없으므로 가짜 업스케일을 하지 않으며, 원본이 Gateway의 4MiB 제한을 넘을 때만 분석 전송용 파생 JPEG를 별도로 만듭니다.
- 동영상은 입력 720×1280·최대 30fps를 그대로 사용한 최대 10초 H.264 High Profile MP4 원본을 보호 저장하지만, OpenClaw에는 MP4를 업로드하지 않습니다. 시간축 대표 프레임 최대 6개의 자연 픽셀 크기를 보존한 2×3 contact sheet를 만들고, JPEG 4MiB 안에서 가장 높은 품질을 선택하며 **대표 프레임 기반 분석**임을 명시합니다.
- 앱 갤러리는 보호된 Application Support 원본을 source of truth로 사용합니다. iPhone 사진 앱에는 `.addOnly` 권한으로 별도 복사하므로 기존 사진 보관함을 읽거나 열거하지 않습니다.
- Photos 복사 실패나 OpenClaw 분석 실패가 앱 원본을 삭제하지 않습니다. 갤러리에서 Photos 저장과 분석을 명시적으로 다시 시도할 수 있습니다.
