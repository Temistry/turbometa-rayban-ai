...
홈의 **OpenClaw** 카드와 Siri의 갈비스 OpenClaw 호출은 모두 전용 갈비스 음성 대화 화면을 엽니다. 이 화면의 **채팅** 버튼으로 전체 답변 이력을 확인하고 텍스트 질문을 보낼 수 있습니다.

앱이 foreground에 있고 사용자가 갈비스 대화 화면을 명시적으로 연 동안에는 첫 질문 이후에도 wake word 없이 **STT → OpenClaw → TTS → STT** 순환을 유지합니다. 무음이나 일시적 음성·오디오 오류는 제한된 backoff 후 마이크를 복구하고, Gateway 연결이 끊기면 재연결한 뒤 새 발화를 기다립니다. timeout이나 연결 오류가 난 질문은 중복 전송을 막기 위해 자동으로 다시 보내지 않으며, 특히 전달 여부가 불명확한 요청은 오류로 종료하고 사용자의 확인을 요구합니다. 앱이 background로 이동하거나 사용자가 종료 문구·종료 버튼을 사용하면 pending STT, OpenClaw 요청, TTS와 오디오 세션을 정리합니다.

#### OpenClaw Quick Shot과 보호 갤러리

홈 화면 최상단의 **사진**·**동영상** Quick Shot은 선택한 촬영 모드의 이름과 프롬프트를 촬영 시작 시점에 고정합니다. 모드는 앱에서 생성·편집·복제·삭제·정렬할 수 있고, 사진과 동영상의 기본/최근 모드를 각각 유지합니다.

- DAT SDK 0.5.0의 세로 `.high` 720×1280은 유지하면서 15fps로 설정합니다. Bluetooth 한정 대역폭을 프레임 수보다 프레임당 세부 묘사에 우선하는 정책이며, 소비자용 Meta 카메라 앱의 12MP/3K 촬영 경로와는 다른 DAT 개발자 스트림입니다.
- 사진은 DAT SDK가 제공한 JPEG 원본을 보호 저장합니다. SDK에는 12MP 사진 해상도 선택 API가 없으므로 가짜 업스케일을 하지 않으며, 원본이 Gateway의 4MiB 제한을 넘을 때만 분석 전송용 파생 JPEG를 별도로 만듭니다. 일반 OpenClaw 채팅의 카메라 버튼도 스트림 화면 캡처 대신 이 정지 사진 JPEG 경로를 사용합니다.
- 동영상은 입력 720×1280·최대 15fps를 그대로 사용한 최대 10초 H.264 High Profile MP4 원본을 보호 저장하지만, OpenClaw에는 MP4를 업로드하지 않습니다. 시간축 대표 프레임 최대 6개의 자연 픽셀 크기를 보존한 2×3 contact sheet를 만들고, JPEG 4MiB 안에서 가장 높은 품질을 선택하며 **대표 프레임 기반 분석**임을 명시합니다.
- 앱 갤러리는 보호된 Application Support 원본을 source of truth로 사용하고 세로 영상의 실제 비율과 원본 해상도·용량·실제 fps·추정 bitrate를 표시합니다. iPhone 사진 앱에는 `.addOnly` 권한으로 별도 복사하므로 기존 사진 보관함을 읽거나 열거하지 않습니다.
- Photos 복사 실패나 OpenClaw 분석 실패가 앱 원본을 삭제하지 않습니다. 갤러리의 명시적 동영상 분석 재시도는 thumbnail이 아니라 보호 원본 MP4에서 contact sheet를 다시 만듭니다.
- **촬영 위치 포함**은 기본적으로 꺼져 있습니다. 사용자가 켜고 iOS의 앱 사용 중 위치 권한을 허용한 경우에만 단발성 iPhone GPS snapshot을 사진 GPS EXIF, 동영상 QuickTime 위치 metadata, Photos asset과 OpenClaw 분석 문맥에 함께 적용합니다. 위치 권한 거부·제한·timeout은 촬영을 막지 않으며 좌표를 진단 로그나 문서에 남기지 않습니다.
- 앱 갤러리 항목 삭제는 앱 원본·썸네일·색인만 삭제합니다. iPhone 사진 앱에 이미 복사된 항목은 삭제하지 않습니다.
- 전송 결과가 불명확하면 자동 재전송하지 않고 **전달 여부 확인 필요**로 남깁니다. 사용자가 중복 가능성을 확인한 뒤 명시적으로 재시도할 때만 새 요청을 만듭니다.

분석 완료 답변은 보호된 OpenClaw 채팅 기록, final 중복 방지, 로컬 알림 및 요약 TTS 경로를 재사용합니다. 앱이 suspend 또는 terminate되어 Gateway final을 처리하지 못하면 알림이나 낭독을 보장하지 않습니다.
...
`DEBUG` 빌드에는 전체 개발자 도구가 포함됩니다. 이 브랜치의 TestFlight 빌드는 제한된 `TESTFLIGHT_TTS_DIAGNOSTICS` 조건으로 보호된 진단 콘솔과 **OpenClaw로 TTS 진단** 경로만 포함하며, 일반 mock/debug 설정은 노출하지 않습니다.
...
- 전체, 오류, 경고 필터
- 검색과 자동 스크롤
- redaction이 적용된 진단 파일 생성·공유
- 로그 전체 삭제
- 읽지 않은 오류 개수 표시

로그 정책:

- 화면에는 최근 최대 2,000줄을 표시
- 내부/TestFlight 진단은 재현 후 분석을 위해 앱의 보호된 Application Support에 제한된 순환 로그를 보관
- Bearer 토큰, API Key, Gateway 토큰, URL 쿼리 자격 증명, UUID·MAC·액세서리 식별정보와 socket/certificate dump 자동 마스킹
- 이미지와 오디오의 긴 Base64 payload 생략
- 사용자 음성 transcript와 OpenClaw 답변 원문은 진단 로그에 기록하지 않음
- 개발자는 진단 파일을 자동 수집하거나 원격 업로드하지 않음
- OpenClaw 진단 전송과 파일 공유는 사용자가 범위를 확인하고 직접 실행해야 함

진단은 기기·액세서리 상태를 디버깅 목적으로 기기 안에서 처리할 수 있습니다. 자동 마스킹은 보조 방어이므로 공유 전에는 사용자 대화, 서버 오류 문구, 호스트 이름 등 주변 정보가 남지 않았는지 직접 확인해야 합니다.

OpenClaw 최종 답변 알림에는 Markdown·URL·코드를 제거한 최대 3문장·250자의 요약본만 넣습니다. 앱이 Gateway final을 실제 처리하고 실행 가능한 동안에는 같은 요약본을 현재 iPhone 미디어 출력(Ray-Ban 또는 iPhone)으로 직접 읽습니다. 일반 로컬 알림만으로 iOS의 시스템 **알림 읽어주기**를 강제할 수 없으며, 앱이 suspend 또는 terminate되어 final을 받지 못하면 알림 생성과 앱 직접 낭독도 시작할 수 없습니다.
