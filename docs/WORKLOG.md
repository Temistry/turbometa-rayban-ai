...
## 2026-09-26 · 2단계: 대화 분석 화면 재설계(Mobbin 보류, iOS 표준 패턴 기준)

### 결정
- 모빈 참조는 유료 계정 문제로 보류하고 iOS 표준 패턴으로 진행.
- 앱 정체를 "회의 통역기"에서 "대화 분석"으로 문구 전환(홈·시작·종료·서랍 ko/en).
- 전사문과 잡아낸 항목을 한 화면에서: 화자 구분 엔진(Gemini)과 전사 엔진(Apple)이 달라 문장이 어긋나므로
  완전 일치 대신 단어 겹침(내용어 60% 이상)으로 전사문에 연결하고, 겹친 단어에 종류색 밑줄.
- 잡아낸 것 색: 근거 없는 단정 주황 · 논리 비약 보라 · 앞뒤 모순 빨강 · 사실 확인 파랑 · 전문용어 노랑(유지).
- 종류별 필터 칩(전체 + 종류별 개수)으로 전사 스트림 필터링.
- 상단: 상태 + 입력 경로 + 경과 시간(1초 갱신) + 목소리 등록 상태 칩.
- 대화 종료 시 요약 리포트 시트: 대화 시간·발화·잡아낸 것 개수·예상 비용 + 카드 목록 + 텍스트 내보내기.
- 잡아낸 항목을 보관소(서랍)에도 저장: ArchivedMeeting.catches(선택 필드, 기존 파일 호환), 상세 화면에 카드 표시.
- 녹취록 내보내기 머리글 "TurboMeta 회의 녹취록" → "TurboMeta 대화 녹취록", 잡아낸 것 섹션 추가.

### 완료
- [x] CatchHighlighter(단어 겹침 연결 + 밑줄 오프셋, 순수 로직)
- [x] MeetingSummaryBuilder(요약·내보내기, 순수 로직), GeminiUsageLedger.totalEstimatedCost
- [x] ArchivedMeetingCatch 모델 + 보관 왕복·내보내기, ArchivedCatchRow
- [x] MeetingModeView 재설계(상단 칩·필터·인라인 강조·카드·요약 시트), VM 상태 노출
- [x] 테스트: 하이라이트 3건 + 요약 2건(ConversationCatchTests), 보관 완전 갱신(MeetingArchiveTests)
- [x] 감사 통과(치명 0 / 경고 0, 참고 1 기존 항목)

### 남음
- [ ] 실기기: 필터 칩·색 밑줄·요약 리포트 확인
- [ ] 단어 겹침 60% 기준이 실제 화자 구분 인용과 Apple 전사문에서 얼마나 연결되는지 로그로 확인

### 보안 메모
- 키·대화 원본 미포함. UI·모델·테스트 변경만.
