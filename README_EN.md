...
- The app keeps DAT SDK 0.5.0 portrait `.high` 720×1280 but runs it at 15fps. This prioritizes per-frame detail over frame count on the constrained Bluetooth link. It is a DAT developer stream, not the consumer Meta camera app's 12MP/3K capture path.
- Photos preserve the JPEG returned by DAT in protected app storage. DAT exposes no 12MP photo-resolution option, so the app does not fake detail by upscaling. Only an original that exceeds the Gateway's 4MiB cap gets a separate analysis-only JPEG derivative. The normal OpenClaw chat camera also uses this still-photo JPEG path instead of a stream screenshot.
- Videos preserve the 720×1280 input at up to 15fps in a bounded H.264 High Profile MP4 original for up to 10 seconds. The app does **not** upload that MP4 to OpenClaw. It builds a 2×3 contact sheet from up to six representative frames at their natural pixel size and selects the highest JPEG quality that fits 4MiB, labeling the result as representative-frame analysis.
- The app-owned repository in protected Application Support is the Gallery source of truth. Gallery respects portrait video aspect ratio and displays original dimensions, bytes, actual fps, and estimated bitrate. Copies to iPhone Photos use add-only access; the app does not read or enumerate the existing photo library.
- A Photos export or OpenClaw analysis failure never removes the protected original. Explicit video-analysis retries rebuild the contact sheet from the protected MP4 instead of using its thumbnail.
- **Include Capture Location** is off by default. Only after the user enables it and grants when-in-use permission does the app take a one-shot iPhone GPS snapshot and apply it to photo GPS EXIF, QuickTime video location metadata, the Photos asset, and the OpenClaw analysis context. Denial, restriction, or timeout never blocks capture, and coordinates are not written to diagnostics or documentation.
